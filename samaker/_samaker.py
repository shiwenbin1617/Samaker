...
import os
import importlib
from typing import List, Dict, Callable, Text, Tuple, Union, Any
...
from samaker.cache import cache
from samaker.log import logger
from samaker.path import BASEDIR
from samaker.exceptions import FileNotFound, YamlKeyError, JsonPathExtractFailed
from samaker.hook_manager import _cli_hook, _session_hook
from samaker.models import ExecuteAsyncJobCondition
...
def command(name, **out_kwargs):
    def decorator(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            from samaker.cli import main, OptionHandler
...
if __name__ == '__main__':
    x = data_maker('samaker/data/api_data/job.yaml', 'job', 'submit_job')
    print(x)
